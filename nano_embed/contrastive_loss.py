...
class ContrastiveLoss(nn.Module):
    ...
    def forward(self, embeddings_a, embeddings_b):
        ...
        cosine_sim_ab = (
            torch.matmul(embeddings_a, embeddings_b.transpose(0, 1)) / self.temperature
        )

        # Similarities within embeddings_a for negative pairs
        cosine_sim_aa = (
            torch.matmul(embeddings_a, embeddings_a.transpose(0, 1)) / self.temperature
        )

        # Similarities within embeddings_b for negative pairs
        cosine_sim_bb = (
            torch.matmul(embeddings_b, embeddings_b.transpose(0, 1)) / self.temperature
        )
        ...
        all_embeddings = torch.cat(
            [embeddings_a, embeddings_b], dim=0
        ...
        full_similarity_matrix = (
            torch.matmul(all_embeddings, all_embeddings.transpose(0, 1))
            / self.temperature
        )
        ...
        labels = torch.cat(
            [
                torch.arange(batch_size, 2 * batch_size).to(
                    embeddings_a.device
                ),  # targets for embeddings_a are corresponding embeddings_b
                torch.arange(batch_size).to(
                    embeddings_a.device
                ),  # targets for embeddings_b are corresponding embeddings_a
            ],
            dim=0,
        )  # (2*batch_size)
        ...
        full_similarity_matrix = full_similarity_matrix.masked_fill(
            mask, -1e9
        )  # Fill diagonal with large negative value
