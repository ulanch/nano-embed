...
def mask_tokens(tokens, mask_ratio, vocab_size, mask_token_id):
    ...
    random_words = torch.randint(
        vocab_size, labels.shape, dtype=torch.long, device=tokens.device
    )
    ...
    labels[~masked_indices] = (
        -100
    )  # -100 is the default ignore_index for F.cross_entropy
...
def tokenizing_distributed_data_loader_with_state(
    ...
    while True:
        ...
        if mask_ratio > 0.0:
            inputs_masked, targets_masked = mask_tokens(
                scratch[:-1].clone(), mask_ratio, vocab_size, mask_token_id
            )
...
def tokenizing_distributed_data_loader(*args, mask_ratio=0.0, **kwargs):
    ...
    for inputs, targets, state_dict in tokenizing_distributed_data_loader_with_state(
        *args, mask_ratio=mask_ratio, **kwargs
    ):
