"""
Train model for Masked Next Token Prediction (MNTP). Run as:

python base_train_mntp.py

or distributed as:

torchrun --nproc_per_node=8 base_train_mntp.py
"""
...
wandb_run = DummyWandb() if use_dummy_wandb else wandb.init(project="nano_embed", name=run, config=user_config)
...
train_loader = tokenizing_distributed_data_loader_with_state(device_batch_size, max_seq_len, split="train", device=device, resume_state_dict=dataloader_resume_state_dict, mask_ratio=mask_ratio)
build_val_loader = lambda: tokenizing_distributed_data_loader(device_batch_size, max_seq_len, split="val", device=device, mask_ratio=mask_ratio)
...
# ... (rest of the code)
    ...
    for micro_step in range(grad_accum_steps):
        with autocast_ctx:
            # MNTP loss
            loss = model(x, y, loss_reduction='mean', is_mntp=True)
